 ...
 * 
 * 
 *  1) OR REPLACE 옵션 : (덮어쓰기)
 * 		기존에 동일한 이름의 VIEW가 존재하면 이를 변경
 * 		없으면 새로 생성
 * 
 *  2) FORCE | NOFORCE 옵션 : 
 *    FORCE(강제로) : 서브쿼리에 사용된 테이블이 존재하지 않아도 뷰 생성
...
AS SELECT EMP_ID 사번, EMP_NAME 이름, NVL(DEPT_TITLE,'없음') 부서명, 
		  JOB_NAME 직급명, NVL(LOCAL_NAME, '없음') 지역명
	FROM EMPLOYEE	
	NATURAL JOIN JOB
	LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
...
--DEPARTMENT 테이블을 복사한 테이블 DEPT_COPY2 생성
...
MAXVALUE 150	-- 증가 가능한 최대값을 150으로 지정
NOMINVALUE		-- 최소값 없음
NOCYCLE			-- 최대값 까지 증가 후 반복 (앞에 NO를 붙히면 반복 X)
NOCACHE			-- 미리 발생시킬 숫자를 만들어 둠
;
...
--> 인덱스가 설정된 컬럼을 WHERE 조건절에 명시하면 됨

SELECT * FROM EMPLOYEE
WHERE EMP_NAME != '0'; -- 3ms

-- EMP_ID (PK) -> 자동으로 인덱스 생성됨
SELECT * FROM EMPLOYEE
WHERE EMP_NAME != '0'; -- 2ms
...
WHERE TEST_ID = 'TEST500000'; -- 19/20/21/17 ms

-- 인덱스 사용 O
SELECT * FROM TB_INDEX_TEST
WHERE TEST_NO = 500000; -- 1/1/1/0 ms
