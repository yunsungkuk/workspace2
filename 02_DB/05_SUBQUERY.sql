...
-- 1) 전 직원의 평균 급여 조회하기
SELECT AVG(SALARY) FROM EMPLOYEE; -- 약 3047663원
...
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
JOIN JOB USING(JOB_CODE)
WHERE HIRE_DATE =  -- (특정 부서 입사일이 가장 빠른사람 SELECT )

	(SELECT MIN(HIRE_DATE) 
	 FROM EMPLOYEE SUB -- 서브 별칭
...
--WHERE ROWNUM = 2; -- 2행만 조회
WHERE ROWNUM BETWEEN 1 AND 10; --> 1부터 포함되야한다.
