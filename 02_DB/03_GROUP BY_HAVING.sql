...
-- 부서코드 순으로 정렬
SELECT DEPT_CODE , COUNT(BONUS) /* BOUNS가 NULL인 행 빼고 카운트 */
FROM EMPLOYEE
GROUP BY DEPT_CODE 
ORDER BY DEPT_CODE ;
...
FROM EMPLOYEE
-- WHERE EXTRACT (YEAR FROM HIRE_DATE) >= 2000 -- EXTRACT문을 이용한 2000년도 이후 입사자들 찾기
WHERE HIRE_DATE >= '2000-01-01' -- 2000-01-01 이후 입사자들 찾기
GROUP BY JOB_CODE;
...
-- 부서 코드 오름차순으로 정렬하세요
SELECT DEPT_CODE , ROUND(AVG(SALARY)) 급여평균
FROM EMPLOYEE
-- WHERE SUBSTR(EMP_NO,1,2) >= 70 AND SUBSTR(EMP_NO,1,2) < 80
WHERE SUBSTR(EMP_NO,1,1) = '7'
-- WHERE EMP_NO LIKE '7%' -- 앞이7 뒤는 무엇이든 상관없다
