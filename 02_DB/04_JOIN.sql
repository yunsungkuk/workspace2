...
-- DEPARTMENT의 DEPT_ID는 의미가 같은 컬럼(값도 같음)
...
--    NULL      = (D1~D9)(+)
-- DEPT_CODE의 값(NULL)이
-- DEPT_ID의 값 (D1 ~ D9)과 일치하지 않아도 추가
...
-- RIGHT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID); -- LEFT JOIN (24행, 마케팅부, 국내영업부, 해외영업3부)
FULL JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID); -- LEFT JOIN (26행, 하동운, 이오리, 마케팅부, 국내영업부, 해외영업3부)
...
SELECT EMP_NAME 사원명 , DEPT_TITLE 사원명 , LOCAL_NAME 지역명 , NATIONAL_NAME 국가명
FROM EMPLOYEE
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE)
