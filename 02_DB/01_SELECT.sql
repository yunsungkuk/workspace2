...
-- 1) 테이블 선정
-- 2) 테이블 한 행씩 접근하면서 조건이 TRUE이면 결과에 포함
--    FALSE면 결과에 미포함
-- 3) 추려진 결과 행 중 컬럼만 선택 -> 최종 RESULT SET 생성
...
-- 사번, 이름, 입사일, 급여 조회
SELECT EMP_ID 사번 , EMP_NAME 이름 , HIRE_DATE AS 입사일 , SALARY AS "급여 조회" 
FROM EMPLOYEE
...
--> 11행이 조회되야 되는데 9행만 조회되었다!
-- 왜?? DEPT_CODE가 NULL인 행이 2행이 있어서 그렇다!
...
-- EMPLOYEE 테이블에서 성이 '수' 로 끝나는 사원의 사번, 이름 조회
SELECT EMP_ID , EMP_NAME 
FROM EMPLOYEE
WHERE EMP_NAME LIKE '%수';

-- EMPLOYEE 테이블에서 성이 '하' 가 포함되는 사원의 사번, 이름 조회
SELECT EMP_ID , EMP_NAME 
FROM EMPLOYEE
WHERE EMP_NAME LIKE '%하%';

-- EMPLOYEE 테이블에서 성이 '전' 시작, '돈' 끝나는 사원의 사번, 이름 조회
SELECT EMP_ID , EMP_NAME 
FROM EMPLOYEE
...
SELECT EMP_NAME , SALARY 
FROM EMPLOYEE
ORDER BY SALARY DESC; -- DESC(descending) : 오름차순
...
/*3*/SELECT EMP_ID "사원", EMP_NAME "이름", SALARY * 12 "연봉"
/*1*/FROM EMPLOYEE
