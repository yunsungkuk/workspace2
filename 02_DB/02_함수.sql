...
-- 찾을 시작 위치부터 지정된 찾은 문자열의 시작 위치를 반환
...
-- 이메일 아이디 오름차순으로 조회
SELECT EMP_NAME, SUBSTR(EMAIL, 1, INSTR(EMAIL, '@')) "아이디"
FROM EMPLOYEE
ORDER BY "아이디";
...
SELECT ADD_MONTHS(SYSDATE, -1) "한 달 전", SYSDATE "현재시간", ADD_MONTHS(SYSDATE, 1)  "한 달 후" FROM DUAL;
...
--> 년,월,일,시,분,초 는 오라클에서 지정한 시간 관련 패턴이 아니다!
...
-- DECODE(컬럼명 | 계산식 , 조건1, 결과1, 조건2, 결과2, ....[, 아무것도 만족 안할때])

-- 컬럼명 | 계산식의 값이 일치하는 조건이 있으면
-- 해당 조건 오른쪽에 작성된 결과가 반환된다.

-- 모든 조건을 불만족 시 [, 아무것도 만족 안할때] 가 반환됨.
...
ORDER BY JOB_CODE /*ASC(오름차순) 생략 가능*/;
...
SELECT SUM(SALARY)		 -- 3) 모인 행들의 급여 합을 조회
FROM EMPLOYEE			 -- 1) EMPLOYEE 테이블에서
WHERE DEPT_CODE = 'D6';  -- 2) DEPT_CODE가 'D6'인 행들만 모아서
...
-- 모든 사원 중
-- 가장 빠른 입사일, 최근 입사일
-- 이름 오름차순에서 제일 먼저 작성되는 이름, 제일 마지막에 작성되는 이름
SELECT MIN(HIRE_DATE), MAX(HIRE_DATE), MIN (EMP_NAME), MAX(EMP_NAME) 
FROM EMPLOYEE;

-- 모든 사원의 급여 합계, 평균, 최대값, 최소값
SELECT SUM(SALARY) 합계, AVG(SALARY) 평균, MAX(SALARY) 최대값, MIN(SALARY) 최소값
FROM EMPLOYEE;

-----------------------------------------------------------------------------

-- ** 실직적으로 사용하는 것이니 꼭 알아두기 (*, 컬럼명)
-- COUNT (*| [DISTINCT] 컬럼명) : 조회된 행의 개수를 반환

-- COUNT(*) : 조회된 모든 행의 개수를 반환

-- COUNT(컬럼명) : 지정된 컬럼에서 NULL이 아닌 행의 개수를 반환
				-- (NULL인 행 미포함)
...
-- EMPLOYEE 테이블에 존재하는 남자,여자 사원을 합한 수
SELECT COUNT (DECODE(SUBSTR(EMP_NO,8,1), '1', '남자', '2', '여자'))
FROM EMPLOYEE;
