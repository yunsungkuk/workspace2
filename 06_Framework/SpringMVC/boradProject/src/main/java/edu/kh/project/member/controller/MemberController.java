...
//	@RequestMapping(value="/login", method = RequestMethod.POST)
	public String login(HttpServletRequest req) {
		...
		/* - Aragument Resolver(매개 변수 확인자(해결사))
		 * 매개변수에 원하는 타입을 선언하면
...
//	@PostMapping("/login")
	public String login(/* @ModelAttribute */ Member member) {
		 ...
		 * 
		 * *******************************************************
		 * @ModelAtttribute로 인해서 필드에 값이 세팅된 객체를
		 * [커맨트 객체]라고 한다
		 * *******************************************************
		 * */
	...
	public String login(Member inputMember
		...
		// -------------------------------------------------------------------
		// 로그인 실패 시 특정 메시지를 *잠깐* session에 세팅하여
		// 메인페이지에서 alert()로 출력하기
		
		// *잠깐* session에 세팅하는 이유 : 
		// 1) redirect 시 request scope 이용 불가
		// 2) session에 계속 있으면 계속 메시지가 출력되기 때문에
		
		/* RedriectAttributes */
		// - 값 세팅 시 : request scope
		// - redriect 중 : session scope 이동
	...
	public String signup(Member inputMember, 
		...
		int result = service.singup(inputMember, memberAddress);
	...
	* */
	 ...
	 * 
	 * // model : 값 전달 객체 (기본 request scope) model.addAttribute("e", e);
	 * 
