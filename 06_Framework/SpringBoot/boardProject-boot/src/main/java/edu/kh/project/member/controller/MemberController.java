...
public class MemberController {
	...
	public String login(Member inputMember, 
		...
		if(loginMember != null) {
			...
			if(saveId != null) {
				...
				// 쿠키의 수명(유지기간) 지정 (초 단위)
				cookie.setMaxAge(60 * 60 * 24 * 30); // 일단 15초(테스트)
	...
	public String signup(Member inputMember, 
		...
		return "redirect:singup"; // 회원 가입 페이지 (상대경로 작성)
...
//		log.debug("memberEmail : " + memberEmail);
		...
		// (기본값) request scope
		// @SessionAttribues({loginMember}) -> session scope 이동
		model.addAttribute("loginMember", loginMember);
