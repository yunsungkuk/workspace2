...
memberEmail.addEventListener("input", () => {
   ...
   if(memberEmail.value.trim().length == 0){
      memberEmail.value = '';
      emailMessage.innerText = "메일을 입력 받을 수 있는 이메일을 입력해주세요.";
...
authKey.addEventListener("input", () => {
   ...
   if(authKey.value.trim().length == 6){
      ...
      authKeyMessage.classList.remove("error"); // 빨간색 글씨제거
      checkObj.authKey = true;
   }
   
   // 입력한 인증번호가 6글자 미만인 경우 (유효 X)
   // --> 유효하지 않은 인증번호 입니다.(빨간색)
   // checkObj.authKey = false;
   else {
      authKeyMessage.innerText = "유효하지 않은 인증번호 입니다.";
      authKeyMessage.classList.add("error"); // 빨간색 글씨
      authKeyMessage.classList.remove("confirm"); // 초록색 글씨제거
      checkObj.authKey = false;
   }
   ...
   for(let key in checkObj) {
      ...
      if( !checkObj[key] ){
         ...
         switch(key) {
            ...
            case "memberNickname" : str = "닉네임이 일치하지 않습니다."; break;
            case "memberTel" : str = "전화번호가 일치하지 않습니다."; break;
         }
