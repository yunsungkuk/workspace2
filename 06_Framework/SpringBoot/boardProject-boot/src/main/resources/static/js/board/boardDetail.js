...
boardLike.addEventListener("click", e => {
   ...
   //     단, Thymeleaf를 작성할 수 있는 html 파이렝서는 js와 혼용 가능
...
if(goToListBtn != null) { // 화면에 목록으로 버튼이 있을 때만 수행
   ...
   const goToListFn = () => {
      ...
      // 상세페이지 주소 : /board/{boardCode}/{boardNo}?cp=1&key=t&query=test

      // 목록 페이지 주소 : /board/{boardCode}?cp=1&key=t&query=test

      // URLSearchParams 객체 : 주소에서 쿼리스트링만 다루는 객체
      //                     쿼리 스트링 추가, 수정, 제거, 생성 가능
