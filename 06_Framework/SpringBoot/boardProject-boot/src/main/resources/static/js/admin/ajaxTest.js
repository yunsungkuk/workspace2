...
selectMemberNo.addEventListener("click", () => {
   ...
   .then( (response) => { // response : 응답이 담겨져있는 객체(promise)
      console.log(response); // promise객체
      return response.text(); // promise에 담긴 응답 결과를 text 형태로 파싱 반환
   } )
   ...
   .then( (email) => { // email : 첫 번째 then 에서 파싱된 데이터
      ...
      if(email == '') {
         result1.innerText="일차하는 회원이 없습니다.";
      } else {
...
/* 문자열을 입력 받아 이메일이 일부라도 일치하는 모든 이메일 조회 */
  ...
  .then(memberList => {
    ...
    for(let member of memberList){ // 향상된 for문
      ...
      btn.addEventListener("click", e => {
         // 버튼에 속성 중 "date-flag" 값이 'N' 이면 'Y', 'Y'이면 'N' 저장
