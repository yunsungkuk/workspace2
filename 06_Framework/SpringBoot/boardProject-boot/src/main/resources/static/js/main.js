...
const getCookie = (key) => {
   ...
   // 현재 존재하는 
   const cookies = document.cookie; // 'saveId@naver.com'
   ...
   // cookies 문자열을 ';' 구분자로해서 배열 후 생성 확인
   // map을 이용해서 cooikes의 요소를 '='로 나누어 2차원 배열 생성
...
// 로그인 form태그가 있을 경우(로그인 X 상태)
if(loginFrm != null){

   // 로그인 form태그가 submit 되는 것을 감지 했을 때 함수 수행
   loginFrm.addEventListener("submit", e => {
      ...
      if(memberPw.value.trim().length == 0){
         ...
         memberPw.focus(); // 비밀번호 input에 커서 깜빡이게 하기
