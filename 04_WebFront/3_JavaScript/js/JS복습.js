...
function checkBingo(){
   ...
   // 3. 대각선(diagonal) 빙고 검사
   diaCheck(board);
}

// 행 빙고 검사
function rowCheck(board){
   // borad : 빙고판을 나타내는 2차원 배열
   // borad[i] : i번째 행
   // borad[i][k] : i번째 행의 k번째 열
...
/* 색상 입력 시 빙고판 글자색 변경 */
fontColor.addEventListener("keyup", () => {
   // bingoBoard.style.color = e.target.value;

   const colors = document.querySelector("#bingo-board");
