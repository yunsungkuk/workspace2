...
check2.addEventListener("click", () => {
   ...
   console.log(`jajajaja : ${regEx.test("jajajaja")}`); // true
...
const telReslut = document.querySelector("#tel-result");

telNumber.addEventListener("keyup", () => {
   const regEx = /^01[0,1,6,7,9]-\d{3,4}-\d{4}$/;

   // 입력 받은 값
   const value = telNumber.value.trim(); // 좌우 공백 제거 상태
   telNumber.value = value; // 좌우 공백이 제거된 값을 대입

   if(value.length == 0) { // 입력 X
      telReslut.innerText = "휴대폰 번호 입력(-포함)";
      telReslut.classList.remove("check"); // check 클래스 제거
      telReslut.classList.remove("error"); // error 클래스 제거
   }

   if(regEx.test(value)){ // 유효한 경우
      telReslut.innerText = "유효한 형식입니다";
      telReslut.classList.add("check"); // check 클래스 추가
      telReslut.classList.remove("error"); // error 클래스 제거
   } else { // 유효하지 않은 경우
      telReslut.innerText = "유효하지 않은 형식입니다"
      telReslut.classList.remove("check") // check 클래스 제거
      telReslut.classList.add("error") // error 클래스 추가
   }
});
