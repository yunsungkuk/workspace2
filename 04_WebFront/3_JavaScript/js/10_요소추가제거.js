...
// 만들려는 기능 중 공통적으로 사용되는 요소를 전역 변수로 선언
...
addBtn.addEventListener("click", () => {
   ...
   span.addEventListener("click", e => {
      ...
      //    부모 요소를 선택(탐색)
      const parent = e.target.parentElement; // == div.rpow
...
calcBtn.addEventListener("click", () => {
   ...
   // 3) for문으로 모든 요소 접근하여 작성된 값(value)를 얻어와 숫자로 변경(Number())한 후
   //    합계 저장 변수 sum에 누적
   let sum = 0;
