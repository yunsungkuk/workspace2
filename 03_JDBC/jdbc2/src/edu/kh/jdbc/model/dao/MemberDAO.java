...
// DAO(Date (Database) Access Object) : 데이터베이스 접근 객체
// - JDBC를 이용해서 DB에 SQL을 전달하여 수행 수 결과를 반환 받는 역할
...
//				+ "	  DEFAULT, DEFAULT)";
		...
		try {
			...
			// 6. 사용한 JDBC 객체 자원 반환
			// (PreparedStement는 Statement의 자식이다!)
			close(pstmt);
		}
	...
	public int updatePw(Connection conn, Member member) {
		...
		try {
			...
			// 4. ? 에 알맞은 값 세팅하기
			pstmt.setString(1, member.getUpdatePw());
