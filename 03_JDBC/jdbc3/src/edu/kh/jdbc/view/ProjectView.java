...
//		int result = service.login(member);
	...
	private void selectAllMember () {
		...
		System.out.println("내림차순 조회(1), 오름차순(아무 숫자) : ");
		int sort = sc.nextInt();
...
//		if(memberList.isEmpty()) { // 안에 비어있나 확인하는 구문(isEmpty)
			System.out.println("\n조회 결과가 없습니다\n");
		} else {
	...
	private void updateMember () {
		...
		int result = service.updateMember(nick, tel, loginMember.getMemberNo());
	...
	private void insertBoard () {
		...
		System.out.println("내용 입력 (입력 종료 : !wq입력)");
	...
	public void selectBoardList () {
		System.out.print("*** 게시글 목록 조회 ***");
		
		System.out.println("최신글 순(1), 반대(나머지숫자) : ");
		int letest = sc.nextInt();
		
		List<Board> boardList = service.selectBoardList(letest);
		// 조회 결과가 없을 경우
		if(boardList.size() == 0) {
//		if(boardList.isEmpty()) { // 안에 비어있나 확인하는 구문(isEmpty)	
			System.out.println("\n조회 결과가 없습니다.\n");
		} else { // 향상된 for문으로 리스트에 모든 값 출력
			for (Board test : boardList) {
