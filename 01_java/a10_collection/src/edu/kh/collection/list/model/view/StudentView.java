package edu.kh.collection.list.model.view;
...
public class StudentView {
	...
	public void displayMenu() {
		...
		int input = 0; // 입력된 메뉴 번호 저장 -> 저장용 공간 생성
	...
	private void addStudent() {
		...
		// 서비스 객체의 addSutdent() 호출(값 6개 전달) 후 boolean 값 반환
		boolean result = service.addSutdent(grade, ban, number, name, gender, score);
	...
	private void selectAllStudent() { 
		...
		// service로 부터 List<Sutdent> 객체의 참조 주소를 반환 받기
		List<Student> studentList = service.selectAllStudent(); // studentList
	...
	/**
	 * 학생 1명을 조회하는 view 메소드
	 */
	
	private void selectOne() {
		System.out.println("\n----- 학생 1명 조회 -----");
		
		System.out.println("조회할 학생의 index 입력 : ");
		...
		// std == null 또는 학생 1명(Student) 반환 받기
	...
	private void deleteStudent() {
		...
		// 인덱스 번호를 입력 받아서 Service 메소드를 호출
		...
		System.out.println("삭제할 학생의 index 입력 : ");
		int index = sc.nextInt();
	...
	private void selectGender() {
		...
		// M 또는 F를 입력한 경우
		// Service의 성별 조회 메소드 호출 (입력 받은 성별 전달)
	...
	private void duplicateValidation() {
		...
		if (result) {
			...
			System.out.println("같은 학생 없음 XXXXXXXXXXXX");
		}
	}
	...
	private void sortScore() {
		...
		List<Student> sortScore = service.sortScore();
		
		for (Student s : sortScore) {
			System.out.println(s);
		}
