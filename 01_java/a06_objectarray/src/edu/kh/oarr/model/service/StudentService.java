...
public class StudentService {
	...
	public StudentService() {
		...
		studentArr[2] = new Student(3,7,20, "박카스");
	...
	public Student selectIndex(int index) {
		...
		// index번째 요소에 학생 객체를 참조하는 조소가 있는 경우
		// 그 주소를 반환
	...
	public void updateStudent(Student s, int kor, int eng, int math) {
		...
		//return이 안써져있는게 아니라 void는 안써져있으면 컴파일러가 알아서 추가 해줌
	}

	/** 학생 이름으로 조회 서비스 */
	public Student[] selectName(String name) {
		
		// 기본 자료형 ||(또는) 참조형(주소) || null 비교 시
		// 비교 연산자 ( ==, != )
	...
	public Student[] selectMaxMin() {
		...
		// 인덱스를 사용한 for문을 쓸땐 향상된 for문을 쓰면 안됨
		for (int i=0 ; i<studentArr.length ; i++) {
			...
			// 최고점의 합
			Student s1 = studentArr[max]; // 짧게 쓸수있게 바꿔놓음 << 이게 없으면 studentArr[Max]을 s1자리에 써야함
